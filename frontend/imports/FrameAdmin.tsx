...
export default function FrameAdmin() {
  ...
  useEffect(() => {
        ...
        const handleUpdate = async () => {
            ...
            isFetching = true;
            try { await fetchAdminData(true); } finally { isFetching = false; }
        };
    ...
    return (
        <div className="w-full min-h-screen flex flex-col items-center justify-center bg-gray-100 p-4">
            <div className="w-full max-w-sm md:max-w-md p-8 bg-white rounded-xl shadow-md text-center border border-red-300">
                ...
                <p className="mt-4 text-sm text-gray-600">{error || "Could not load admin data."}</p>
                    ...
                    <div className="w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className="w-full mt-12">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 lg:gap-6">
                                <ActionButton onClick={() => navigate('/security')} variant="secondary" icon={LockKeyholeIcon}>Security</ActionButton>
                                <ActionButton onClick={() => navigate('/users')} variant="secondary" icon={Users}>Manage Users</ActionButton>
                                <ActionButton onClick={() => navigate('/view-logs')} variant="secondary" icon={FileText}>View Logs</ActionButton>
                            </div>
                        </div>       
