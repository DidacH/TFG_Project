...
export default function FrameUsers() {
    ...
    const handleDownload = async (type: 'users' | 'logs') => {
        ...
        try {
            ...
            if (!response.ok) {
                const errorText = await response.text();
                throw new Error(`Codi ${response.status}: ${errorText}`);
            }
            ...
            alert(`Error de descàrrega: ${err.message}`);
        } finally {
            setDownloading(null);
        }
    };
    ...
    const filteredUsers = users.filter(user => {
        const matchesSearch = user.name.toLowerCase().includes(searchTerm.toLowerCase()) || 
                              ...
                              user.id.toLowerCase().includes(searchTerm.toLowerCase()); // Ara també busca per ID
    ...
    return (
        <div className="flex flex-col min-h-screen w-full bg-background overflow-hidden">
            ...
            {/* Main Content Area - Constrained to screen height for internal scrolling */}
            <div className="flex-grow w-full flex flex-col gap-4 px-4 sm:px-6 lg:px-10 pb-6 pt-8 md:pt-12 mt-20 h-[calc(100vh-80px)]">
                
                {/* Toolbar (Stays fixed relative to the page) */}
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-4 rounded-xl border border-gray-200 shadow-sm shrink-0">
                    ...
                    <button 
                        ...
                        className="flex items-center justify-center gap-2 px-4 py-2 bg-[#eeeeee] hover:bg-[#e0e0e0] text-black font-medium rounded-lg transition-colors w-full md:w-auto"
                    >
                        {downloading === 'users' ? <Loader2 className="w-5 h-5 animate-spin" /> : <Download className="w-5 h-5" />}
                        Export CSV
                    </button>
                </div>

                {/* Table Container - Scrollable area with sticky header */}
                <div className="overflow-y-auto overflow-x-auto bg-white rounded-xl border border-gray-200 shadow-sm flex-grow mb-4 relative">
                    {loading ? (
                        <div className="flex justify-center items-center h-40"><Loader2 className="w-10 h-10 animate-spin text-[#c8102e]" /></div>
                    ) : error ? (
